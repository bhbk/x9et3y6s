    ...
    public class DiagnosticControllerTest
    {
        ...
        public void Admin_DiagV1_GetStatus_Success()
        {
            using (var client = _factory.CreateClient())
            {
                ...
                var result = controller.GetStatusV1(Enums.TaskType.QueueEmails.ToString().ToLower()) as OkObjectResult;
                var ok = result.Should().BeOfType<OkObjectResult>().Subject;
                var data = ok.Value.Should().BeAssignableTo<string>().Subject;

                result = controller.GetStatusV1(Enums.TaskType.QueueTexts.ToString()) as OkObjectResult;
