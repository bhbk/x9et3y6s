  ...
  loadData(): void {
    ...
    forkJoin({
      ...
      next: ({ users, activity }) => {
        ...
        this.lockedAccountCount.set(String(users.data.filter(u => u.isLockedOut).length));
        this.pendingConfirmationCount.set(String(users.data.filter(u => !u.emailConfirmed).length));
