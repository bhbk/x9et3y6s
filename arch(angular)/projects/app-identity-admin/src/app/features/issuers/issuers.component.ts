...
import { IssuerService, IssuerV1, IssuerCreate, IssuerUpdate } from 'lib-identity';
import { KENDO_GRID, GridDataResult, PageChangeEvent, PagerSettings, SortSettings, FilterableSettings } from '@progress/kendo-angular-grid';
  ...
  template: `
    <div class="p-6">
      ...
      <div class="flex justify-between items-center mb-6">
        <div>
          <p class="text-gray-500">Manage client issuers and their settings</p>
        </div>
      ...
      @if (showDialog()) {
        <kendo-dialog
          title="{{ isEditing() ? 'Edit Issuer' : 'Create Issuer' }}"
          (close)="closeDialog()"
          [minWidth]="400"
          [width]="500"
        >
...
export class IssuersComponent implements OnInit {
  ...
  loadData(): void {
    ...
    this.issuerService.getAll(query).subscribe({
      next: (result) => {
        this.gridData.set({
          data: result.data,
          total: result.total
  ...
  openEdit(issuer: IssuerV1): void {
    this.isEditing.set(true);
    this.editingId = issuer.id;
    this.form.patchValue({
      name: issuer.name,
      description: issuer.description || '',
      isEnabled: issuer.isEnabled,
      isDeletable: issuer.isDeletable
    });
    this.showDialog.set(true);
  }
  ...
  saveIssuer(): void {
    ...
    if (this.isEditing() && this.editingId) {
      const update: IssuerUpdate = {
        id: this.editingId,
        ...formValue
      };

      this.issuerService.update(update).subscribe({
        next: () => {
        ...
        error: (err) => {
          this.isSaving.set(false);
          this.error.set(err.error?.message || err.message || 'Failed to update issuer');
        }
      });
    } else {
      const create: IssuerCreate = formValue;

      this.issuerService.create(create).subscribe({
        next: () => {
          this.isSaving.set(false);
          this.closeDialog();
          this.loadData();
        ...
        error: (err) => {
          this.isSaving.set(false);
          this.error.set(err.error?.message || err.message || 'Failed to create issuer');
        }
      });
    }
  }

  confirmDelete(issuer: IssuerV1): void {
    this.pendingDeleteId = issuer.id;
    this.pendingDeleteName.set(issuer.name);
  ...
  executeDelete(): void {
    ...
    this.deleteIssuer(id);
  }

  private deleteIssuer(id: string): void {
    this.isLoading.set(true);
    ...
    this.issuerService.delete(id).subscribe({
      ...
      error: (err) => {
        this.isLoading.set(false);
        this.error.set(err.error?.message || err.message || 'Failed to delete issuer');
      }
    });
  }
}
