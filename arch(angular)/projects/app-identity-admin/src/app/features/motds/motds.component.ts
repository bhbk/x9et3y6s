...
import { MotdService, AdminMotdService, MOTDTssV1 } from 'lib-identity';
  ...
  template: `
    <div class="p-6">
      <div class="flex justify-between items-center mb-6">
        <div>
          <h1 class="text-2xl font-semibold text-gray-800">Messages of the Day</h1>
          <p class="text-gray-500">Inspirational quotes and messages for users</p>
        </div>
...
export class MotdsComponent implements OnInit {
  ...
  private readonly adminMotdService = inject(AdminMotdService);
    ...
    this.motdService.getMOTD().subscribe({
      ...
      error: () => {
        const fallbackQuote: MOTDTssV1 = {
          ...
          author: 'System',
          quote: 'Welcome to the Identity Management System. Configure your MOTD service to display inspirational quotes.',
          category: 'system',
          tags: ['welcome']
        };
  ...
  loadAllQuotes(): void {
    ...
    this.adminMotdService.getAll(query).subscribe({
      next: (result) => {
        this.gridData.set({
          data: result.data,
          total: result.total
