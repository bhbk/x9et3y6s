...
namespace Bhbk.WebApi.Identity.Admin
{
    public class Startup
    {
        ...
        public virtual void ConfigureServices(IServiceCollection sc)
        {
            ...
            var workerName = "IdentityAdminWorker";
            ...
            var llmSettings = LoadLLMProviderSettings(conf["Databases:IdentityEntities_EF"], "Admin");
            sc.AddSingleton(Microsoft.Extensions.Options.Options.Create(llmSettings));

            RegisterKeyedLLMProvider(sc, llmSettings, "Admin");
            ...
            sc.AddQuartz(jobs =>
            {
                ...
                if (jobSettings.TryGetValue("Maintain Activity", out var maintainActivity) && maintainActivity.IsEnabled)
                {
                    ...
                    foreach (var cron in maintainActivity.Schedules)
                    {
