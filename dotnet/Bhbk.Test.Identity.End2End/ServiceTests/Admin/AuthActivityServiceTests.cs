...
{
    public class AuthActivityServiceTests : IClassFixture<BaseAdminServiceTests>
    {
        private readonly BaseAdminServiceTests _factory;

        public AuthActivityServiceTests(BaseAdminServiceTests factory) => _factory = factory;
        ...
        public async Task Admin_AuthActivityV1_Get_Success()
        {
            ...
            using (var scope = _factory.Server.Host.Services.CreateScope())
            {
                ...
                var state = new PagerState()
                {
                    ...
                    {
                        new PagerStateSort() { Field = "createdUtc", Dir = "asc" }
                    },
                    Skip = 0,
                    Take = take
                };
