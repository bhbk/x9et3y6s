    ...
    public class RoleServiceTests : IClassFixture<BaseAdminServiceTests>
    {
        ...
        public async Task Admin_RoleV1_Update_Success()
        {
            ...
            using (var scope = _factory.Server.Host.Services.CreateScope())
            {
                ...
                var testRole = uow.Roles.GetAsNoTracking(x => x.Name == _factory.TestData.Role.Name).Single();
                testRole.Description += "(Updated)";

                var result = await service.Role_UpdateV1(map.Map<RoleV1>(testRole));
                result.Should().BeAssignableTo<RoleV1>();
