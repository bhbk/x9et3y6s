...
{
    public class ClaimServiceTests : IClassFixture<BaseAdminServiceTests>
    {
        private readonly BaseAdminServiceTests _factory;

        public ClaimServiceTests(BaseAdminServiceTests factory) => _factory = factory;
        ...
        public async Task Admin_ClaimV1_Delete_Fail()
        {
            ...
            using (var scope = _factory.Server.Host.Services.CreateScope())
            {
                ...
                using var seed = ScenarioMother.CreateClaimSeed(uow);
                ...
                var testClaim = seed.Claims.Values.First();
                testClaim.IsDeletable = false;

                uow.Claims.Put(testClaim);
                uow.Commit();
        ...
        public async Task Admin_ClaimV1_Delete_Success()
        {
            ...
            using (var scope = _factory.Server.Host.Services.CreateScope())
            {
                ...
                /* no using - the test deletes the claim */
                var seed = ScenarioMother.CreateClaimSeed(uow);
                var testClaimId = seed.Claims.Values.First().Id;
                ...
                var result = await service.Claim_DeleteV1(testClaimId);
                result.Should().BeTrue();

                var check = uow.Claims.Get(QueryExpressionFactory.GetQueryExpression<tbl_Claim>()
                    .Where(x => x.Id == testClaimId).ToLambda())
                    .Any();
        ...
        public async Task Admin_ClaimV1_Get_Success()
        {
            ...
            using (var scope = _factory.Server.Host.Services.CreateScope())
            {
                ...
                var state = new PagerState()
                {
                    ...
                    {
                        new PagerStateSort() { Field = "type", Dir = "asc" }
                    },
                    Skip = 0,
                    Take = take
                };
        ...
        public async Task Admin_ClaimV1_Update_Success()
        {
            ...
            using (var scope = _factory.Server.Host.Services.CreateScope())
            {
                ...
                var testClaim = uow.Claims.GetAsNoTracking(QueryExpressionFactory.GetQueryExpression<tbl_Claim>()
                    .Where(x => x.Id == seed.Claims.Values.First().Id).ToLambda())
                    .Single();
