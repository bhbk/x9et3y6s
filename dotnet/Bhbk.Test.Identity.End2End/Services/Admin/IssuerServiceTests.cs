...
{
    public class IssuerServiceTests : IClassFixture<BaseAdminServiceTests>
    {
        private readonly BaseAdminServiceTests _factory;

        public IssuerServiceTests(BaseAdminServiceTests factory) => _factory = factory;
        ...
        public async Task Admin_IssuerV1_Delete_Fail()
        {
            ...
            using (var scope = _factory.Server.Host.Services.CreateScope())
            {
                ...
                // Create a test issuer that is not deletable
                using var seed = ScenarioMother.CreateIssuerSeed(uow);
                ...
                var testIssuer = seed.Issuer;
                testIssuer.IsDeletable = false;

                uow.Issuers.Put(testIssuer);
                uow.Commit();
        ...
        public async Task Admin_IssuerV1_Delete_Success()
        {
            ...
            using (var scope = _factory.Server.Host.Services.CreateScope())
            {
                ...
                // Create a test issuer to delete
                using var seed = ScenarioMother.CreateIssuerSeed(uow);
        ...
        public async Task Admin_IssuerV1_Get_Success()
        {
            ...
            using (var scope = _factory.Server.Host.Services.CreateScope())
            {
                ...
                var result = await service.Issuer_GetV1(testIssuer.Id.ToString());
                result.Should().BeAssignableTo<IssuerV1>();
            }
            ...
            using (var scope = _factory.Server.Host.Services.CreateScope())
            {
                ...
                // Create a test issuer to ensure we have data for paging
                using var seed = ScenarioMother.CreateIssuerSeed(uow);
        ...
        public async Task Admin_IssuerV1_GetKeys_Success()
        {
            ...
            using (var scope = _factory.Server.Host.Services.CreateScope())
            {
                ...
                // Create a test issuer for key retrieval
                using var seed = ScenarioMother.CreateIssuerSeed(uow);
                ...
                service.Grant.AccessToken = auth.ClientCredential(issuer.Name, issuer.IssuerKey, conf["IdentityTenant:Salt"], audience.Name, cc_claims);

                var testIssuer = seed.Issuer;
                var testList = new List<string> { issuer.Id.ToString(), testIssuer.Id.ToString() };
        ...
        public async Task Admin_IssuerV1_Update_Success()
        {
            ...
            using (var scope = _factory.Server.Host.Services.CreateScope())
            {
                ...
                // Create a test issuer to update
                using var seed = ScenarioMother.CreateIssuerSeed(uow);
                ...
                var testIssuer = uow.Issuers.GetAsNoTracking(x => x.Id == seed.Issuer.Id).Single();
