...
namespace Bhbk.Lib.Identity.Services
{
    public class AdminService : IAdminService
    {
        ...
        public async ValueTask<PagerStateResult<AuthActivityV1>> Activity_GetV1(PagerState model)
        {
            var response = await Endpoints.Activity_GetV1(Grant.AccessToken.RawData, model);

            if (response.IsSuccessStatusCode)
                return await response.Content.ReadAsAsync<PagerStateResult<AuthActivityV1>>();
