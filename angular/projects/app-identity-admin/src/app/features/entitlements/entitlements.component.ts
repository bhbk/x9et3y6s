...
import {
  EntitlementService, UserEntitlementV1, UserEntitlementCreate, UserEntitlementUpdate,
  EntitlementTypeV1, EntitlementScopeV1, UserService, UserV1,
  IssuerService, IssuerV1, AudienceService, AudienceV1
} from 'lib-identity';
  ...
  template: `
    <div>
      ...
      <div class="flex justify-between items-center mb-6">
        <div>
          <p class="text-gray-500">Manage user entitlements for application-level access control</p>
        </div>
      ...
      <div class="bg-white rounded-lg border border-gray-200">
        ...
        >
          <kendo-grid-column field="userName" title="User" [width]="180" [sortable]="false">
          ...
          <kendo-grid-column field="audienceName" title="Audience" [width]="150" [sortable]="false">
            <ng-template kendoGridCellTemplate let-dataItem>
              <span class="text-sm text-gray-500">{{ dataItem.audienceName || '-' }}</span>
            </ng-template>
          </kendo-grid-column>
      ...
      @if (showCreateDialog()) {
        ...
        >
          <form [formGroup]="createForm" class="p-2">
            <div class="space-y-4">
              <div>
                <kendo-label text="User" for="userId"></kendo-label>
                <kendo-dropdownlist
                  id="userId"
              ...
              @if (selectedScopeName() === 'Issuer' || selectedScopeName() === 'Audience') {
                <div>
                  ...
                  <kendo-dropdownlist
                    ...
                    [style.width.%]="100"
                    [valuePrimitive]="true"
                    (valueChange)="onIssuerChange($event)"
                  ></kendo-dropdownlist>
                </div>
              }

              @if (selectedScopeName() === 'Audience') {
                <div>
      ...
      @if (showDeleteDialog()) {
        <kendo-dialog
          ...
          [width]="420"
        >
          <p>Are you sure you want to delete this entitlement for "{{ pendingDeleteUserName() }}"? This action cannot be undone.</p>
          <kendo-dialog-actions>
...
export class EntitlementsComponent implements OnInit {
  ...
  readonly pendingDeleteUserName = signal('');
  ...
  private loadLookups(): void {
    ...
    this.userService.getAll({ take: 1000, sort: [{ field: 'userName', dir: 'asc' }] }).subscribe({
      next: (result) => { this.users.set(result.data); checkReady(); },
      error: (err) => { this.error.set(err.message || 'Failed to load users'); this.isInitialLoad.set(false); }
    });
    ...
    this.audienceService.getAll({ take: 1000, sort: [{ field: 'name', dir: 'asc' }] }).subscribe({
      next: (result) => { this.audiences.set(result.data); this.filteredAudiences.set(result.data); checkReady(); },
  ...
  loadData(): void {
    ...
    this.entitlementService.getAll(query).subscribe({
      ...
      error: (err) => {
        this.error.set(err.message || 'Failed to load entitlements');
        this.isLoading.set(false);
        this.isInitialLoad.set(false);
      }
    });
  ...
  onUserFilterChange(filter: string): void {
    if (!filter) {
      this.userService.getAll({ take: 1000, sort: [{ field: 'userName', dir: 'asc' }] }).subscribe({
        next: (result) => this.users.set(result.data)
      });
    } else {
      this.userService.getAll({
        take: 50,
        filter: { logic: 'or', filters: [{ field: 'userName', operator: 'contains', value: filter }] }
      }).subscribe({
        next: (result) => this.users.set(result.data)
      });
    }
  ...
  onScopeChange(scopeId: string): void {
    ...
    if (scope?.name === 'Global') {
      this.createForm.patchValue({ issuerId: null, audienceId: null });
    } else if (scope?.name === 'Issuer') {
      this.createForm.patchValue({ audienceId: null });
    }
  }

  onIssuerChange(issuerId: string): void {
    this.createForm.patchValue({ audienceId: null });
    if (issuerId) {
  ...
  saveCreate(): void {
    ...
    const create: UserEntitlementCreate = {
      ...
      audienceId: formValue.audienceId || undefined,
  ...
  openEdit(entitlement: UserEntitlementV1): void {
  ...
  saveEdit(): void {
    ...
    const update: UserEntitlementUpdate = {
      id: this.editingId,
      isEnabled: formValue.isEnabled,
      isDeletable: formValue.isDeletable
    };
  ...
  confirmDelete(entitlement: UserEntitlementV1): void {
    this.pendingDeleteId = entitlement.id;
    this.pendingDeleteUserName.set(entitlement.userName || 'Unknown');
  ...
  closeDeleteDialog(): void {
    ...
    this.pendingDeleteUserName.set('');
  }
