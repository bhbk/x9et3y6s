...
import { LLMProviderService, LLMProviderV1, LLMProviderSettingV1 } from 'lib-identity';
    ...
    <div class="p-6">
      @if (isInitialLoad()) {
        ...
        <div class="flex justify-between items-center mb-6">
          <div>
            <p class="text-gray-500">Manage LLM providers and failover priority</p>
          </div>
          @if (hasOrderChanged()) {
...
export class LlmProvidersComponent implements OnInit {
  ...
  readonly providers = signal<LLMProviderV1[]>([]);
  ...
  onDrop(event: CdkDragDrop<LLMProviderV1[]>): void {
    ...
    current.forEach((p, i) => p.failoverOrder = i + 1);
  ...
  saveOrder(): void {
    ...
    const order = this.providers().map(p => ({ id: p.id, failoverOrder: p.failoverOrder }));
