    ...
    public class IssuerServiceTests : IClassFixture<BaseServiceTests>
    {
        ...
        public async Task Admin_IssuerV1_GetKeys_Fail()
        {
            ...
            using (var scope = _factory.Server.Host.Services.CreateScope())
            {
                ...
                result = await service.Http.Issuer_GetKeysV1(rop.RawData, new List<string> { string.Empty });
                result.Should().BeAssignableTo(typeof(HttpResponseMessage));
                result.StatusCode.Should().Be(HttpStatusCode.Forbidden);
            }
        }
        ...
        public async Task Admin_IssuerV1_GetKeys_Success()
        {
            ...
            using (var scope = _factory.Server.Host.Services.CreateScope())
            {
                ...
                var service = new AdminService(conf, instance.InstanceType, owin);
                service.Grant = new ClientCredentialGrantV2(conf, instance.InstanceType, owin);
                ...
                service.Jwt = auth.ClientCredential(issuer.Name, issuer.IssuerKey, conf["IdentityTenants:Salt"], audience.Name, cc_claims);
