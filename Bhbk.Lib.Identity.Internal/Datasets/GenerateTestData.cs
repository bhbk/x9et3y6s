...
namespace Bhbk.Lib.Identity.Datasets
{
    public class GenerateTestData
    {
        ...
        public async Task CreateAsync()
        {
            ...
            //create roles
            await _uow.RoleRepo.CreateAsync(_uow.Convert.Map<AppRole>(
                new RoleCreate()
                {
            ...
            //create user A
            await _uow.UserRepo.CreateAsync(_uow.Convert.Map<AppUser>(
                new UserCreate()
                {
            ...
            await _uow.UserRepo.SetConfirmedEmailAsync(user, true);
            await _uow.UserRepo.SetConfirmedPasswordAsync(user, true);
            await _uow.UserRepo.SetConfirmedPhoneNumberAsync(user, true);
            await _uow.CommitAsync();
            ...
            await _uow.UserRepo.AddClaimAsync(user, new Claim(Strings.ApiUnitTestClaimType, Strings.ApiUnitTestClaimValue));

            if (!await _uow.UserRepo.IsInRoleAsync(user, role.Name))
                await _uow.UserRepo.AddToRoleAsync(user, role.Name);

            if (!await _uow.UserRepo.IsInLoginAsync(user, login.LoginProvider))
                await _uow.UserRepo.AddLoginAsync(user, new UserLoginInfo(login.LoginProvider, login.LoginProvider, "local"));
        ...
        public async Task DestroyAsync()
        {
            ...
            {
                var userRoles = await _uow.UserRepo.GetRolesAsync(user);

                await _uow.UserRepo.RemoveFromRolesAsync(user, userRoles.ToArray());
            ...
            foreach(var role in roles)
                await _uow.RoleRepo.DeleteAsync(role);
