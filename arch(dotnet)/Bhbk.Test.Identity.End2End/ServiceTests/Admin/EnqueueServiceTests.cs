...
namespace Bhbk.Test.Identity.End2End.AdminServiceTests
{
    public class EnqueueServiceTests : IClassFixture<BaseAdminServiceTests>
    {
        ...
        public EnqueueServiceTests(BaseAdminServiceTests factory) => _factory = factory;
            ...
            using (var scope = _factory.Server.Host.Services.CreateScope())
            {
                ...
                var result = await service.Enqueue_EmailV1(
                    new EmailV1()
                    {
                        ...
                        Subject = _factory.TestData.Email.Subject + "-" + Base64.CreateString(4),
                        Body = _factory.TestData.Email.Content + "-" + Base64.CreateString(4)
                    });
            ...
            using (var scope = _factory.Server.Host.Services.CreateScope())
            {
                ...
                var result = await service.Enqueue_TextV1(
                    new TextV1()
                    {
                        ...
                        Body = _factory.TestData.Text.Content,
                    });
                result.Should().BeTrue();
            }
        }
